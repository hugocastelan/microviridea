            ...
            for record in SeqIO.parse(ruta_archivo, 'genbank'):
                for feature in record.features:
                    if feature.type == 'CDS':
                        ...
                        producto = feature.qualifiers.get('product', ['unknown_product'])[0]
                        # Obtener la secuencia de nucleótidos
                        secuencia = feature.extract(record.seq)
                        # Crear una entrada FASTA
                        fasta_entry = f">{record.id}_{feature.qualifiers.get('locus_tag', [''])[0]}\n{secuencia}\n"
                        productos[producto].append(fasta_entry)
    
    # Crear el directorio de salida si no existe
    os.makedirs(directorio_salida, exist_ok=True)
    
    # Escribir cada grupo de productos homólogos en archivos multifasta separados
    for producto, secuencias in productos.items():
        nombre_archivo = os.path.join(directorio_salida, f"{producto.replace(' ', '_')}.fasta")
        with open(nombre_archivo, 'w') as archivo_salida:
            archivo_salida.write(''.join(secuencias))

# Ejemplo de uso
directorio_entrada = '/home/hugo/costal_marine/output'  # Cambia esto a tu directorio de entrada
directorio_salida = '/home/hugo/costal_marine/salida_2'    # Cambia esto a tu directorio de salida
